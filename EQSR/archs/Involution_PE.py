...
# Last Modified:  Tuesday, 21st December 2021 10:21:29 am
...
class RBF(nn.Module):
    ...
    def __init__(self, **kwargs):
      ...
      self.kernel_base = torch.ones((1, 1, 3, 3, 1, 1),requires_grad=False) * -1

      self.kernel_base[:,:,[0,0,2,2],[0,2,0,2],:,:] *= 1/sqrt(2)
      self.kernel_base[:,:,1,1,:,:] = 0
      lengthscale = torch.tensor(2.0) if lengthscale \
    ...
    def forward(self, scale):
      temp = (scale/self.lengthscale**2)
      # print(temp.shape)
      r2 =  self.kernel_base.to(scale) * temp
      # r2 = self.act(r2)
