...
class DSF_nopre_DF2K_Dataset(data.Dataset):
    def __init__(  
                    self,
                    opt
                ):
        super(DSF_nopre_DF2K_Dataset, self).__init__()
    ...
    def __preprocess__(self):
        ...
        data_path_list = sorted(list(scandir(div2khr_path, suffix="png", recursive=True, full_path=True)))
        ...
        if not os.path.exists('./DF2K_list.pkl'):
            ...
            tmp_file = open("./DF2K_list.pkl", 'wb')
            pickle.dump(self.data_path_list, tmp_file)
            tmp_file.close()
        else:
            print("min pixels:", self.max_scale*self.l_ps)
            self.data_path_list = pickle.load(open( './DF2K_list.pkl', 'rb'))
